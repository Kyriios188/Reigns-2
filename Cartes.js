...
const bandit = new carte("Bandits", "Un groupe de bandits pille nos terres et terrorise notre peuple. Nous devons envoyer notre armée !", "Ne rien faire.", "Envoyer l'armée pour arréter les bandits", effetD_bandit, effetG_bandit, argD_bandit, argG_bandit, "./armee.png");

const mauvaise_recolte = new carte("Mauvaises récoltes", "Monseigneur ! Les récoltes sont mauvaises cette année... Nous pourrions donner moins de pain au peuple ou réduire les rations de la garde, mais il faut choisir.", "Nourrir le peuple", "Nourrir la garde", effetD_mauvaise_recolte, effetG_mauvaise_recolte, argD_mauvaise_recolte, argG_mauvaise_recolte, "./paysan.jpeg");

const potion2 = new carte("Potion", "Souhaiteriez-vous être un leader charismatique monseigneur ? Ou préférez-vous une force surhumaine ? J'ai justement deux potions pour ça...", "Devenir un leader charismatique", "Avoir une force surhumaine", effetD_potion2, effetG_potion2, argD_potion2, argG_potion2, "./potion.png");

const chasse = new carte("Chasse", "La chasse est un succès monseigneur ! Souhaitez-vous organiser un banquet pour célébrer l'occasion ? Nous pourrions inviter le peuple afin d'améliorer votre image ou plutôt le réserver à la bourgeoisie des pays voisins.", "Inviter le peuple", "Inviter la bourgeoisie étrangère", effetD_chasse, effetG_chasse, argD_chasse, argG_chasse, "./chasse.png");
...
const guerre = new carte("Guerre", "Des rapports de nos espions nous informent que la nation voisine renforçait dangereusement son armée près de notre frontière. Nous pourrions leur déclarer la guerre de manière préventive au risque de fragiliser notre propre armée, ou essayer de parlementer.", "Déclarer la guerre", "Parlementer", effetD_guerre, effetG_guerre, argD_guerre, argG_guerre, "./parlementer.jpeg");

const investissement = new carte("Investissement", "Monseigneur, je voudrais ouvrir une taverne, mais il me manque quelques fonds pour le droit de propriété. Si vous vouliez bien investir dans mon projet, nous pourrions tous deux en tirer un sacré bénéfice. Qu'en dites-vous ?", "Investir", "Ne rien faire", effetD_investissement, effetG_investissement, argD_investissement, argG_investissement, "./paysan2.jpeg");

const peche = new carte("Pêche", "Mon roi, nous devrions profiter de ce beau temps pour organiser une grande pêche. Cela permettrait d'offrir une distraction au peuple, mais les éloignerait des champs et de leur travail.", "Partir pêcher", "Laisser le peuple travailler", effetD_peche, effetG_peche, argD_peche, argG_peche, "./paysan2.jpeg");

const relique = new carte("Relique", "Une expédition, envoyée pour explorer la région boisée au Nord-Est, a découvert une étrange boîte dorée. D'après notre abbé, il s'agirait de la relique de Saint Roland. Nous devrions organiser une grande procession pour l'entreposer dans notre cathédrale. Désirez-vous laisser l'Eglise s'en charger ?", "Laisser l'Eglise s'en charger", "Tout organiser", effetD_relique, effetG_relique, argD_relique, argG_relique, "./pretre2.jpeg");

const troubadour = new carte("Troubadour", "Messire ! Permettez-moi de me présenter. Je suis le grand Jean de Boër, troubadour itinérant. Je souhaitais me produire ici quelques temps. Je pourrais vous proposer une partie de mes recettes...", "Accepter sa proposition", "Lui laisser l'intégralité de ses recettes", effetD_troubadour, effetG_troubadour, argD_troubadour, argG_troubadour, "./troubadour.jpeg");

const trahison_general = new carte("Trahison du général", "Mon roi ! Votre général a comploté contre vous ! Il a rallié plusieurs officiers à sa cause et s'apprête à vous renverser. Nous devons agir !", "Exécuter les traîtres", "Envoyer le général à la guerre dans une contrée lointaine", effetD_trahison_general, effetG_trahison_general, argD_trahison_general, argG_trahison_general, "./armee.png");

const barbares = new carte("Barbares", "Des barbares pillent nos églises et tuent nos prêtres ! Envoyez immédiatement l'armée pour les protéger !", "Protéger les prêtres", "Ignorer leur problème", effetD_barbares, effetG_barbares, argD_barbares, argG_barbares, "./pretre2.jpeg");

const voleur = new carte("Voleur", "Nous avons surpris un voleur qui s'était introduit dans notre cathédrale. Il tenait dans ses mains une de nos reliques, et son sac était rempli d'argenterie. Il semblerait que cet homme soit une sorte de héro du peuple qui volerait aux riches. Un certain Borin des forêts. Nous devrions l'enfermer dans nos geôles pour l'exemple...", "Emprisonner Borin des Forêts", "Relâcher Borin des Forêts", effetD_voleur, effetG_voleur, argD_voleur, argG_voleur, "./voleur.jpeg");

const joutes = new carte("Joutes", "C'est le festival du printemps seigneur ! Bien que cela représente un coût, nous pourrions organiser des joutes pour divertir le peuple.", "Organiser les joutes", "Ne rien organiser", effetD_joutes, effetG_joutes, argD_joutes, argG_joutes, "./joute.jpeg");

const gisement = new carte("Gisement", "Nous avons découvert un gisement de minerai. Vous pourriez utiliser le métal ainsi extrait pour équiper l'armée ou pour notre économie.", "Créer de la monnaie", "Créer des armes et de l'équipement", effetD_gisement, effetG_gisement, argD_gisement, argG_gisement, "./paysan2.jpeg");

const academie_magie = new carte("Construction d'une académie de magie", "Nous pourrions construire une académie de magie et d'alchimie mon seigneur. Evidemment, nous ferions profiter la couronne des profits générés par l'alchimie...", "Construire l'académie", "Ne rien construire", effetD_academie_magie, effetG_academie_magie, argD_academie_magie, argG_academie_magie, "./academie_magie.jpeg");

const banque = new carte("Construction d'une banque", "Nous pourrions construire une banque pour vous monseigneur. Elle vous permettrait de réaliser un bon profit.", "Construire la banque", "Ne rien construire", effetD_banque, effetG_banque, argD_banque, argG_banque, "./banque.jpeg");

const cachots = new carte("Construction de cachots", "Le taux de criminalité dans nos villes est trop élevé. Vous devriez ordonner la création de cachots pour emprisonner les criminels.", "Construire les cachots", "Ne rien construire", effetD_cachots, effetG_cachots, argD_cachots, argG_cachots, "./armee.png");

const caserne = new carte("Construction d'une caserne", "Mon roi, nous pourrions construire une caserne et enrôler de nouvelles recrues pour renforcer nos défenses.", "Construire une caserne", "Ne rien construire", effetD_caserne, effetG_caserne, argD_caserne, argG_caserne, "./armee.png");

const cathedrale = new carte("Construction d'une cathédrale", "La foi du peuple est vascillante, je le sens ! Nous devrions ordonner la construction d'une grande cathédrale à la gloire des dieux.", "Construire la cathédrale", "Ne rien construire", effetD_cathedrale, effetG_cathedrale, argD_cathedrale, argG_cathedrale, "./pretre2.jpeg");

const grenier = new carte("Construction d'une grenier", "Nos récoltes sont régulièrement à l'origine de surplus, mais sans moyen de les entreposer, ils sont perdus. Nous devrions construire un grenier pour les entreposer.", "Construire un grenier", "Ne rien construire", effetD_grenier, effetG_grenier, argD_grenier, argG_grenier, "./paysan2.jpeg");

const taverne = new carte("Construction d'une taverne", "Le peuple s'ennuie monseigneur... Nous pourrions construire une taverne afin de leur changer les idées.", "Construire une taverne", "Ne rien construire", effetD_taverne, effetG_taverne, argD_taverne, argG_taverne, "./paysan2.jpeg");

const tour_guet = new carte("Construction d'une tour de guet", "Nous subissons trop souvet des incursions des pays voisins ou des barbares qui profitent d'une faible sécurité aux frontières pour les traverser. Nous devrions construire des tours de guet afin de surveiller les limites de notre territoire.", "Construire une tour de guet", "Ne rien construire", effetD_tour_guet, effetG_tour_guet, argD_tour_guet, argG_tour_guet, "./armee.png");

const intro = new carte("Bienvenue Monseigneur", "Bienvenue jeune roi ! Votre objectif va être de survivre le plus longtemps possible. Vous allez être mis face à une succession de décisions et chaque choix aura des conséquences. Pour prendre une décision, il vous suffit de cliquer sur le côté gauche ou droit de la carte. Les quatres jauges représentent les différents pouvoirs qui influencent le royaume. A vous de faire attention à ce que vos décisions n'amènent pas une des ces jauges à 0 ou au contraire au maximum, sinon vous vous ferez destituer et ce sera le game over. Bonne chance !","...","...",effetD_intro,effetG_intro,argD_intro,argG_intro,"./intro.png");
